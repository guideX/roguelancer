    ...
    public class Fadeout : IParticleAffector {
        ...
        public void Affect(GameTime gameTime, DynamicParticle particle) {
            if(particle.Age.HasValue) {
                particle.Color = Color.Lerp(particle.InitialColor, new Color((byte)particle.InitialColor.R, (byte)particle.InitialColor.G, (byte)particle.InitialColor.B, (byte)0), 1.0f - particle.Age.Value);
            }
        }
    }
}
