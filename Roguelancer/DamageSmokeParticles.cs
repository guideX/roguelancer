...
namespace Roguelancer
{
    ...
    public class DamageSmokeParticles
    {
        ...
        public void Emit(Vector3 position, Vector3 shipVelocity)
        {
            int particleCount = _random.Next(1, 3); // Emit a few particles at a time
            ...
            for (int i = 0; i < particleCount; i++)
            {
                ...
                p.Life = 0f;
                p.MaxLife = (float)(_random.NextDouble() * 3.0 + 2.0); // Smoke lingers
                p.InitialSize = (float)(_random.NextDouble() * 3 + 2);
        ...
        public void Update(float deltaTime)
        {
            for (int i = _particles.Count - 1; i >= 0; i--)
            {
                ...
                p.Size = p.InitialSize * (1f + lifeRatio * 1.5f); // Smoke expands
