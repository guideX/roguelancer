...
namespace Roguelancer.Collections {
    ...
    public class ShipCollection : IShipCollection {
        ...
        public void Reset(RoguelancerGame game) {
            ...
            ShipObject tempShip;
            playerShip.Model.WorldObject = game.Settings.Model.StarSystemSettings[game.CurrentStarSystemId - 1].Model.Ships.Where(s => s.Model.SettingsModelObject.ModelType == ModelTypeEnum.Ship && s.Model.SettingsModelObject.IsPlayer).FirstOrDefault();
            playerShip.ShipModel.PlayerShipControl.Model.UseInput = true;
            Model.Ships.Add(playerShip);
            foreach (var modelWorldObject in game.Settings.Model.StarSystemSettings[game.CurrentStarSystemId - 1].Model.Ships.Where(s => !s.Model.SettingsModelObject.IsPlayer).ToList()) {
                tempShip = new ShipObject(game);
                tempShip.Model = new GameModel(game, null, null, ModelTypeEnum.Ship, "Ship");
